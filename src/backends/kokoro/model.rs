...
use super::phonemizer::{EspeakConfig, phonemize, voice_lang};
...
impl KokoroModel {
    ...
    /// - An `.onnx` file (preferably `model_q8f16.onnx`)
    /// - A `voices/` directory with onnx-community `.bin` voice files,
    ///   or a legacy `voices-v1.0.bin` voice archive
    ...
    pub fn load(
        ...
        // Load voices
        let voice_store = VoiceStore::load(model_dir)?;
...
///
/// Prefers onnx-community model names, then the first `.onnx` file found.
pub(super) fn find_onnx_file(model_dir: &Path) -> Result<std::path::PathBuf, KokoroError> {
    for name in [
        "model_q8f16.onnx",
        "model.onnx",
        "model_fp16.onnx",
...
mod tests {
    ...
    #[test]
    fn find_onnx_file_prefers_model_q8f16() {
        let dir = temp_model_dir("prefers-q8f16");
        std::fs::write(dir.join("model.onnx"), []).expect("write fp32 model");
        std::fs::write(dir.join("model_q8f16.onnx"), []).expect("write q8f16 model");

        let path = find_onnx_file(&dir).expect("find onnx");

        assert_eq!(
            path.file_name().and_then(|name| name.to_str()),
            Some("model_q8f16.onnx")
        );
        std::fs::remove_dir_all(dir).expect("remove temp model dir");
    }

    #[test]
    fn find_onnx_file_falls_back_to_other_onnx() {
        let dir = temp_model_dir("falls-back-onnx");
        std::fs::write(dir.join("custom.onnx"), []).expect("write custom model");
